...
## Phase 2.5：Output Completeness Rules（必填規則，觸發時不得省略）

以下兩條規則在觸發條件成立時，必須產出對應 derived_metric claim。缺少視為輸出不完整，系統會自動標記警告。

### OC-1：非常態項目對毛利率影響（Gross Margin Adjustment）

觸發條件：claims 中出現 recurring=false 且與毛利相關的項目（例如：存貨跌價迴轉利益、存貨跌價損失、廠房減損影響毛利、不動產出售利益）

必填 derived_metric：
- claim：「[項目名] 對毛利率影響 = [金額] ÷ [當期營收] = [百分比]；排除此項目後，調整後毛利率 ≈ [X]%（計算自 p.XX）」
- section_key：accounting_adjustments
- materiality：tier_a（若調整幅度 ≥ 3%）；否則 tier_b
- recurring：false
- confidence：high（若數字直接引自原文）

⚠️ 此條 claim 必須單獨輸出，不得與其他 claim 合併。

### OC-2：現金安全墊（Liquidity Safety Margin）

觸發條件：claims 中同時出現：
- 現金及約當現金（liquidity）
- 加上任意一項：短期借款、流動負債可轉債、已宣告/應付股利

必填 derived_metric：
- claim：「現金安全墊 = 現金 [X] - 近期義務合計（[項目 A] + [項目 B] + ...）[Y] = [Z] 千元（計算自 p.XX）」
- section_key：liquidity
- materiality：tier_a
- recurring：true（現金安全墊是流動性常態指標）
- confidence：high

⚠️ 若現金安全墊為負值或 < 5%，claim 需加註 requires_human_review: true。
...
- 只陳述數字變化事實，不解釋原因
- 若 claims 中存在 OC-1 觸發的調整後毛利率，最後一句必須加：「注意：[項目名]（[金額]千元）為非常態項目，影響本期毛利率與同期可比性。」
